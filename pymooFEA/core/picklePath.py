...
from pymooFEA.util.loggingTools import MultiLineFormatter, DispNameFilter
...
class PicklePath:
    ...
    def save_sub_pickle_paths(self):
        ...
        for attr_key, attr_val in self.__dict__.items():
            ...
            if self.is_iterable(attr_val) and not isinstance(attr_val, str):
                # if (isinstance(attr_val, list) or
                #     isinstance(attr_val, set) or
                #     isinstance(attr_val, tuple)):
                #     for item in attr_val:
    ...
    def load_self(self):
        if os.path.exists(self.cp_path):
            ...
            self.logger.debug('\tRESTART DICTIONARY')
            for key, val in self.__dict__.items():
                self.logger.debug(f'\t\t{key}: {val}')
            self.logger.debug('\tCHECKPOINT DICTIONARY')
    ...
    def update_self(self, loaded_self=None):
        ...
        # UPDATE: only instance dictionary
