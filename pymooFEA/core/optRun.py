...
from pymooFEA.util.sysTools import copy_and_overwrite, saveTxt
...
class OptRun(PicklePath):
    ...
    def run(self, delPrevGen=True):
        ...
        while self.algorithm.has_next():
            ...
            self.plotAllOpt()
            self.plotGens()
            if gen % self.gens_per_plot == 0:
                self.plotGens(gens=range(gen-self.gens_per_plot+1, gen+1))
            if gen == 1:
    ...
    #     automated meshing introducting more variablility into the FEA case
    #     workflow.
    #     '''
    ...
    #     from pymooFEA.execSimsBatch import singleNodeExec
