...
import re
from pymooFEA.core.meshStudy import MeshStudy
from pymooFEA.core.picklePath import PicklePath
from pymooFEA.util.sysTools import saveTxt, yes_or_no
...
class FEACase(PicklePath):  # (PreProcCase, PostProcCase)
    '''
    Notes:
        - FEA cases with external solvers are launched using the subprocess module.
    ...
    def _solve(self):
        self.logger.error(
            'OVERRIDE _solve(self) method to execute internal python solver OR use FEACase.externalSolver=True')
...
class FluentCase(FEACase):
    pass


class OpenFOAMCase(FEACase):
    pass
